...
float4 main(VS_OUTPUT input) : SV_TARGET
{
    ...
    GetDiffuse(mat, lightDirection, ambientLightIntensity.rgb, ambientLightIntensity.a, 1.f, dif);
    diffuse += dif;


    float3 result = diffuse * mat.occlusionColor + mat.emissiveColor;
    result *= colorMul.rgb;
    result += colorAdd.rgb;
    ...
    if(mApplyFog)
    {
        float3 view = cameraPosition.xyz - input.vertexPos.xyz;
