...
public:
    ...
    template<typename ClassDerivingFromSparseSet, typename ...Args>
    ClassDerivingFromSparseSet& GuusEngineAddPool(Args &&...args)
    {
        std::shared_ptr<base_type> newPool = std::make_shared<ClassDerivingFromSparseSet>(std::forward<Args>(args)...);
        ClassDerivingFromSparseSet& returnValue = static_cast<ClassDerivingFromSparseSet&>(*newPool);

        const entt::type_info& typeInfo = newPool->type();
        ENTT_ASSERT(this->storage(typeInfo.hash()) == nullptr, "Cannot addPool, there is already a pool using this typehash");
        pools.emplace(typeInfo.hash(), std::move(newPool));
        return returnValue;
    }
