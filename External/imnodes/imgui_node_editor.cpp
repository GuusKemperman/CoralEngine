...
static void LogV(const char* fmt, va_list args)
{
    ...
    static char mBuffers[1024];

    vsnprintf(mBuffers, buffer_size - 1, fmt, args);
    mBuffers[buffer_size - 1] = 0;

    ImGui::LogText("\nNode Editor: %s", mBuffers);

    OutputDebugStringA("NodeEditor: ");
    OutputDebugStringA(mBuffers);
    OutputDebugStringA("\n");
}
# endif
...
ed::EditorContext::~EditorContext()
{
    ...
    for (auto node  : mNodes)  delete node.m_Object;
...
bool ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    if (availableContentSize.x <= 0.0f)
    {
        return false;
    }

    if (availableContentSize.y <= 0.0f)
    {
        return false;
    }
    ...
    {
        canvasSize.x = availableContentSize.x;
    }

    if (canvasSize.y <= 0.0f)
    {
        canvasSize.y = availableContentSize.y;
    }
    ...
    if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize)
    {
        ...
        previousVisibleRect = m_Canvas.ViewRect();
...
void ed::EditorContext::End()
{
    ...
    if (control.ActiveNode)
    {
        if (!IsGroup(control.ActiveNode))
        {
            ...
            auto activeNodeIt = std::find(mNodes.begin(), mNodes.end(), control.ActiveNode);
            std::rotate(activeNodeIt, activeNodeIt + 1, mNodes.end());
        }
        else if (!isDragging && m_CurrentAction && m_CurrentAction->AsDrag())
        {
            ...
            std::stable_partition(mNodes.begin(), mNodes.end(), [&nodes](Node* node)
            {
    ...
    if (sortGroups || ((m_Settings.m_DirtyReason & (SaveReasonFlags::Position | SaveReasonFlags::Size)) != SaveReasonFlags::None))
    {
        ...
        auto groupsItEnd = std::stable_partition(mNodes.begin(), mNodes.end(), IsGroup);

        // Sort groups by area
        std::sort(mNodes.begin(), groupsItEnd, [this](Node* lhs, Node* rhs)
        {
    ...
    // Apply Z order
    std::stable_sort(mNodes.begin(), mNodes.end(), [](const auto& lhs, const auto& rhs)
    {
...
# if 1
    ...
    {
        // Copy group nodes
        auto liveNodeCount = static_cast<int>(std::count_if(mNodes.begin(), mNodes.end(), [](Node* node) { return node->m_IsLive; }));
        ...
        auto groupsItEnd = std::find_if(mNodes.begin(), mNodes.end(), [](Node* node) { return !IsGroup(node); });

        // Copy group nodes
        std::for_each(mNodes.begin(), groupsItEnd, copyNode);
        ...
        // Copy normal nodes
        std::for_each(groupsItEnd, mNodes.end(), copyNode);
    }
# endif
...
{
    return (int)std::count_if(mNodes.begin(),  mNodes.end(),  [](const Node* node)  { return node->m_IsLive; });
}
...
{
    return FindItemInLinear(mNodes, id);
}
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
    ...
    for (auto nodeIt = mNodes.rbegin(), nodeItEnd = mNodes.rend(); nodeIt != nodeItEnd; ++nodeIt)
    {
        auto node = *nodeIt;
...
{
    mNodes.push_back(NodeSettings(id));
    return &mNodes.back();
}

ed::NodeSettings* ed::Settings::FindNode(NodeId id)
{
    for (auto& settings : mNodes)
        if (settings.m_ID == id)
            return &settings;
...
std::string ed::Config::Load()
{
    ...
    if (LoadSettings)
    {
        const auto size = LoadSettings(nullptr, UserPointer1, UserPointer2);
        if (size > 0)
        {
            data.resize(size);
            LoadSettings(const_cast<char*>(data.data()), UserPointer1, UserPointer2);
        }
    }
...
std::string ed::Config::LoadNode(NodeId nodeId)
{
    ...
    if (LoadNodeSettings)
    {
        const auto size = LoadNodeSettings(nodeId, nullptr, UserPointer1, UserPointer2);
        if (size > 0)
        {
            data.resize(size);
            LoadNodeSettings(nodeId, const_cast<char*>(data.data()), UserPointer1, UserPointer2);
        }
    }
...
{
    if (BeginSaveSession)
        BeginSaveSession(UserPointer1, UserPointer2);
}
...
bool ed::Config::Save(const std::string& data, SaveReasonFlags flags)
{
    if (SaveSettings)
    {
        return SaveSettings(data.c_str(), data.size(), flags, UserPointer1, UserPointer2);
    }
    else if (SettingsFile)
    {
...
{
    if (SaveNodeSettings)
        return SaveNodeSettings(nodeId, data.c_str(), data.size(), flags, UserPointer1, UserPointer2);
...
{
    if (EndSaveSession)
        EndSaveSession(UserPointer1, UserPointer2);
}
