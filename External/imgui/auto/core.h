...
{
	if constexpr (sizeof(T) <= IMGUI_AUTO_TREE_MAX_ELEMENT_SIZE)
	{
...
template<typename Container> void ImGui::detail::AutoContainerPushFrontButton(Container &cont)	{ if(ImGui::SmallButton("Add to start")) cont.emplace_front(); }
template<typename Container> void ImGui::detail::AutoContainerPushBackButton(Container &cont)	{ if(ImGui::SmallButton("Add to end")) cont.emplace_back(); }
template<typename Container> void ImGui::detail::AutoContainerPopFrontButton(Container &cont)	{ if(!cont.empty() && ImGui::SmallButton("Remove from start")) cont.pop_front(); }
template<typename Container> void ImGui::detail::AutoContainerPopBackButton(Container &cont)	{ if(!cont.empty() && ImGui::SmallButton("Remove from end")) cont.pop_back(); }
...
#define IMGUI_AUTO_DEFINE_END }	static constexpr bool sIsSpecialized = true; };}
