...
if constexpr ((std::is_fundamental_v<T1> || std::is_same_v<std::string, T1>) && (std::is_fundamental_v<T2> || std::is_same_v<std::string, T2>))
{
...
IMGUI_AUTO_DEFINE_BEGIN_P((template<typename T1, typename T2>), (const std::pair<T1,T2>))
	...
	if constexpr (std::is_fundamental_v<T1> && std::is_fundamental_v<T2>) ImGui::SameLine();
...
IMGUI_AUTO_DEFINE_BEGIN(template<typename T>, std::vector<T>)
	if(ImGui::detail::AutoContainerValues< std::vector<T> >("Array " + name, var))
	{
