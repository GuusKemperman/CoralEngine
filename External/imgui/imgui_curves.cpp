...
namespace ImGui
{
    ...
    template <typename T>
    static inline constexpr float lerpInv(T min, T max, T value) { return max <= min ? min : (value - min) / (max - min); }
    ...
    int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points)
    {
        ...
        while (max < maxpoints && points[max].x >= 0)
        {
            ...
            graphUpperBound = std::max(graphUpperBound, points[max].y);
            max++;
        }
        ...
        if (hovered)
        {
            ...
            if (g.IO.MouseDown[0])
            {
                ...
                pos.y = ImLerp(graphLowerBound, graphUpperBound, 1.0f - pos.y);
                ...
                p.y /= graphUpperBound - graphLowerBound;

                float p1d = p.x * p.x + p.y * p.y;
                p = points[left + 1] - pos;
                p.y /= graphUpperBound - graphLowerBound;
                float p2d = p.x * p.x + p.y * p.y;
                int sel = -1;
        ...
        float smoothnessValues[smoothness + 1];

        // smooth curve
        for (i = 0; i <= smoothness; ++i)
        {
            const float px = i / float(smoothness);
            const float value = CurveValueSmooth(px, maxpoints, points);

            smoothnessValues[i] = value;
        }

        // smooth curve
        for (i = 0; i <= (smoothness - 1); ++i)
        {
            const float px = (i + 0) / float(smoothness);
            const float qx = (i + 1) / float(smoothness);

            const float curveValueP = smoothnessValues[i];
            const float curveValueQ = smoothnessValues[i + 1];

            const float py = 1.0f - lerpInv(graphLowerBound, graphUpperBound, curveValueP);
            const float qy = 1.0f - lerpInv(graphLowerBound, graphUpperBound, curveValueQ);
        ...
        for (i = 1; i < max; i++)
        {
            ImVec2 a = points[i - 1];
            ImVec2 b = points[i];
            a.y = 1.0f - lerpInv(graphLowerBound, graphUpperBound, a.y);
            b.y = 1.0f - lerpInv(graphLowerBound, graphUpperBound, b.y);
        ...
        if (hovered)
        {
            ...
            {
                ImVec2 p = points[i];
                p.y = 1 - lerpInv(graphLowerBound, graphUpperBound, p.y);
        ...
        if (ImGui::Combo("Ease type", &item, items, IM_ARRAYSIZE(items))) {
            ...
            if (item > 0) {
                for (i = 0; i < max; ++i) {
                    ...
                    points[i].y = float(tween::ease(item - 1, points[i].x)) * (graphUpperBound - graphLowerBound);
                }
                modified = 1;
            }
        }
