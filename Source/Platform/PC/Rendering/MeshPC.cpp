    ...
    Asset(loadInfo)
{
    ...
    std::vector<glm::vec3> tangentStorage(0);
    const glm::vec3* tangents = nullptr;

    if (flags & hasTangents) {
        tangentStorage.resize(numOfVertices);
        str.read(reinterpret_cast<char*>(tangentStorage.data()), numOfVertices * sizeof(glm::vec3));
        tangents = tangentStorage.data();
    }
    ...
    bool meshLoaded = LoadMesh(indices.data(),
        ...
        reinterpret_cast<const float*>(tangents),
        numOfVertices
    );
...
bool Engine::StaticMesh::LoadMesh(const char* indices, unsigned int indexCount, unsigned int sizeOfIndexType, const float* positions, const float* normalsBuffer, const float* textureCoordinates, const float* tangents, unsigned int vertexCount)
{
	...
	int nBufferSize = sizeof(float) * mVertexCount * 4;
	...
	mNormalBufferView.StrideInBytes = sizeof(float) * 4;
