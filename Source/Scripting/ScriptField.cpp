...
CE::ScriptField::ScriptField(const Script& script, const std::string_view name) :
...
CE::ScriptField::ScriptField(ScriptField&& other) noexcept :
...
CE::ScriptField& CE::ScriptField::operator=(ScriptField&& other) noexcept
{
	mTypeData = std::move(other.mTypeData);
	mNameOfScriptAsset = std::move(other.mNameOfScriptAsset);
	mDefaultValue = std::move(other.mDefaultValue);
...
CE::ScriptField::~ScriptField() = default;

void CE::ScriptField::SetType(const MetaType& type)
{
...
bool CE::ScriptField::CanTypeBeUsedForFields(const MetaType& type)
{
...
void CE::ScriptField::SerializeTo(BinaryGSONObject& object) const
{
...
std::optional<CE::ScriptField> CE::ScriptField::DeserializeFrom(const BinaryGSONObject& object, const Script& owningScript, [[maybe_unused]] const uint32 version)
{
...
void CE::ScriptField::CollectErrors(ScriptErrorInserter inserter, const Script& owningScript) const
{
...
CE::MetaAny& CE::ScriptField::GetDefaultValue()
{
...
const CE::MetaAny& CE::ScriptField::GetDefaultValue() const
{
	return const_cast<ScriptField&>(*this).GetDefaultValue();
}

bool CE::ScriptField::WasValueDefaultConstructed() const
{
	if (mDefaultValue.index() == 0)
	{
