...
namespace Engine
{
	...
	void Graph::DebugDrawAStarGraph(const World& world) const
	{
		...
		{
			world.GetDebugRenderer().AddCircle(world, DebugCategory::Gameplay, glm::vec3{n.GetPosition().x, 0, n.GetPosition().y},
			                              0.2f,
			                              {1.f, 0.f, 0.f, 1.f});
			for (const auto& m : n.GetConnectingEdges())
			{
				world.GetDebugRenderer().AddLine(world, DebugCategory::Gameplay, glm::vec3{n.GetPosition().x, 0, n.GetPosition().y},
				                            glm::vec3{
