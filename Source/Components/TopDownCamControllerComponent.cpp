...
void Engine::TopDownCamControllerComponent::ApplyTranslation(TransformComponent& transform, const glm::vec2& cursorDistanceFromScreenCenter) const
{
...
{
	const glm::vec3 direction = target - transform.GetWorldPosition();

	transform.SetLocalOrientation(glm::quatLookAt(direction, sUp));
}
...
Engine::MetaType Engine::TopDownCamControllerComponent::Reflect()
{
	...
	type.AddField(&TopDownCamControllerComponent::mAngle, "mRotation").GetProperties().Add(Props::sIsScriptableTag);
