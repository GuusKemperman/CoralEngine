...
CE::MetaType CE::AbilitiesOnCharacterComponent::Reflect()
{
	...
	BindEvent(metaType, sOnBeginPlay, &AbilitiesOnCharacterComponent::OnBeginPlay);

#ifdef EDITOR
	BindEvent(metaType, sOnInspect, &AbilitiesOnCharacterComponent::OnInspect);
#endif // EDITOR
...
void CE::AbilitiesOnCharacterComponent::OnInspect(World& world, const entt::entity owner)
{
	const Registry& reg = world.GetRegistry();
	isPlayer = reg.HasComponent<PlayerComponent>(owner);
	ShowInspectUI("Abilities", mAbilitiesToInput);
	ShowInspectUI("Weapons", mWeaponsToInput);
}
