...
void Engine::CollisionRules::DisplayWidget(const std::string& name)
{
	if (ImGui::TreeNode(name.c_str()))
	{
		...
		static constexpr uint32 numOfResponses = static_cast<uint32>(CollisionResponse::NUM_OF_RESPONSES);
		...
		if (ImGui::BeginTable("##ResponseTable", numOfResponses + 1))
		{
			...
			for (int layer = 0, id = 213456; layer < static_cast<uint32>(CollisionLayer::NUM_OF_LAYERS); layer++)
			{
				ImGui::TableNextRow();
				ImGui::TableSetColumnIndex(0);
...
Engine::MetaType Engine::PhysicsBody2DComponent::Reflect()
{
	...
	metaType.AddField(&PhysicsBody2DComponent::mRules, "mRules").GetProperties().Add(Props::sIsScriptableTag);
