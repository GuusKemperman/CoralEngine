	...
	*/
	...
	public:
		...
		*/
		...
		void RedirectFunction(InvokeT&& func)
		{
	...
	template <typename ... Args>
	FuncResult MetaFunc::operator()(Args&&... args) const
	{
		return InvokeCheckedUnpackedWithRVO(nullptr, std::forward<Args>(args)...);
	}

	template <typename ... Args>
	FuncResult MetaFunc::InvokeCheckedUnpacked(Args&&... args) const
	{
		return InvokeCheckedUnpackedWithRVO(nullptr, std::forward<Args>(args)...);
	}

	template <typename ... Args>
	FuncResult MetaFunc::InvokeCheckedUnpackedWithRVO(RVOBuffer rvoBuffer, Args&&... args) const
	{
	...
	template <typename ... Args>
	FuncResult MetaFunc::InvokeUncheckedUnpacked(Args&&... args) const
	{
		return InvokeCheckedUnpackedWithRVO(nullptr, std::forward<Args>(args)...);
	}

	template <typename ... Args>
	FuncResult MetaFunc::InvokeUncheckedUnpackedWithRVO(RVOBuffer rvoBuffer, Args&&... args) const
	{
		static constexpr uint32 numOfArgs = sizeof...(Args);
		std::pair<std::array<MetaAny, numOfArgs>, std::array<TypeForm, numOfArgs>> packedArgs = Pack(std::forward<Args>(args)...);
	...
	template <typename Arg>
	MetaAny MetaFunc::PackSingle(Arg&& arg)
	{
